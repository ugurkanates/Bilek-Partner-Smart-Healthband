...
public class Mlogic{
    ...
    public ret whatIsUserDoing(double acx[],double acy[],double acz[],double gcx[],double gcy[], double gcz[]){
        ...
        if(newAve.isaxNeg)
            ++negs;
        if(newAve.aData.az > 0.89 || newAve.aData.az < 0.0001 && lastOdata.lastState != UserState.RUNNING && lastOdata.lastState != UserState.WALKING && lastOdata.lastState != UserState.ON_STAIRS){
