...
public class Chat extends AppCompatActivity implements Bluetooth.CommunicationCallback {
    ...
    Mlogic.UserState instant_state;
    @Override
    ...
    public void Display(final String s){
        this.runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if(counter==2){
                    counter=0;
                    if(instant_state== Mlogic.UserState.STANDING)
                        img.setBackgroundResource(R.drawable.sitting);
                    else if(instant_state== Mlogic.UserState.WALKING)
                        img.setBackgroundResource(R.drawable.walking);
                    else if(instant_state== Mlogic.UserState.RUNNING)
                        img.setBackgroundResource(R.drawable.running);
                    else if(instant_state==Mlogic.UserState.ON_STAIRS)
                ...
                if(StartScreen.packet==10){
                    ...
                    if(bt.batter==0){
                        t4.setText("Batarya Yeterli");
                    }
                    else if(bt.batter==1){
                        t4.setText("Batarya Tam Dolu");
                    }
                    else if(bt.batter==2){
                        t4.setText("Batarya Boş \n Şarja Takınız");
                    }
                }
